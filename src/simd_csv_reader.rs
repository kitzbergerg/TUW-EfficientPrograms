...
pub fn parse_csv(data: &[u8]) -> Vec<(CsvField<'_>, CsvField<'_>)> {
    let mut indices: Vec<usize> = Vec::with_capacity(data.len() / 8);
    let mut pos = 0;
    while pos + CHUNK_SIZE < data.len() {
        ...
        find_indices(&mut indices, pos, combined);
    ...
    let mut start = 0;
    while let (Some(middle), Some(end)) = (iter.next(), iter.next()) {
        let f1 = unsafe { from_raw_parts(data.as_ptr().add(start), middle - start) };
        let f2 = unsafe { from_raw_parts(data.as_ptr().add(middle + 1), end - middle - 1) };
        pairs.push((f1, f2));
...
#[inline(always)]
fn find_indices(indices: &mut Vec<usize>, pos: usize, mut combined: u64) {
    while combined != 0 {
...
#[inline(always)]
fn find_indices(indices: &mut Vec<usize>, pos: usize, combined: u64) {
    ...
    for i in 0..combined.count_ones() {
        indices.push(pos + offsets[i as usize] as usize);
    }
}
